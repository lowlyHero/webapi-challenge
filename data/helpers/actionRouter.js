...
router.get('/:id', async (req, res) => {
    try {
        ...
        if(action) {
            ...
            res.status(404).json({ message: 'Action not found. Please try again.' });
        }
    } catch(error) {
        console.log(error);
        res.status(500).json({ message: 'Internal error. Cannot retrieve data.' });
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if(action) {
            ...
            res.status(404).json({ message: 'Unable to update action. Please try again.' });
        }
    } catch(error) {
        console.log(error);
        res.status(500).json({ message: 'Internal error. Cannot update data.' });
    }
});
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        console.log(error);
        res.status(500).json({ message: 'Internal error. Cannot remove data.' });
    }
});
